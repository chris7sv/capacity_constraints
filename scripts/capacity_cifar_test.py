...
"""
Capacity constraint simulations from Section 3.3, procedure for CIFAR-10
...
if __name__ == '__main__':
    ...
    set_of_x = [10, 100]#[20,200,2000]#10000
    set_of_n = [10]#[2,3,4]#(np.arange(2,10,1))
    # n = args.n_classes
    # x = args.stimuli
    ...
    for n in set_of_n:
        for x in set_of_x:
            ...
            for rep in range(args.repeat):
                ...
                cur_noise = 0.#1. if (args.model == 'alx' and args.act=='relu') else .5 if (args.model == 'inc' and args.act=='relu') \
                            #else .2 if (args.model=='alx' and args.act == 'sigmoid') else .14
                max_noise = (2.0 + 1e-5) if args.act == 'relu' else (0.5 + 1e-5)
                ...
                while cur_noise <= max_noise:
                    ...
                    draw_train = np.empty((0,), dtype='uint16')
                    draw_test = np.empty((0,), dtype='uint16')

                    if n != 10:
                        sample = np.random.choice(range(10), n, replace=False)
                        ...
                        #draw_train = np.zeros(shape=y_train.shape)
                        #draw_test = np.zeros(shape=y_test.shape) 

                        for stim in sample:
                            print(np.where(y_train == stim)[0].shape)                             
                            draw_train = np.concatenate(
                                (draw_train, np.random.choice(
                                    np.where(y_train == stim)[0], size=x,
                                    replace=False)))
                            draw_test = np.concatenate(
                                (draw_test, np.random.choice(
                                    np.where(y_test == stim)[0],
                                    size=int(x/10), replace=False)))

                        x_test, y_test = (x_test[draw_test],
                                          y_test[draw_test])
                        x_train, y_train = (x_train[draw_train],
                                            y_train[draw_train])
                    ...
                    y_train = vectorize_labels(y_train, n)
                    y_test = vectorize_labels(y_test, n)

                    if args.data == 'rnd-labels':
                    ...
                    epochs = min(2500, all_steps_full//max(
                    ...
                    out_tracker = PickOutput(x_train[:2], 29, y_train[:2])   
                    ...
                    pickle.dump(rnd, open(os.path.join(save_dir, 'rnd.p'),
                                          'wb'))
                    plot_results(logs, 'cifar', n, args.model, save_dir)
